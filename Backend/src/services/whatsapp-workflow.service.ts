...
export class WhatsAppWorkflowService {
  ...
  async handleMessage(
    ...
    if (workflowState.step) {
      // Only clear workflow if user is in awaiting_media step and sends unrelated message
      const isAwaitingMedia = workflowState.step === 'awaiting_media';
      const isUnrelatedMessage = !mediaUrl && !postingIntent.detected && message.trim().length < 50;
      
      if (isAwaitingMedia && isUnrelatedMessage) {
        console.log('🔁 User sent a non-posting request while awaiting media. Clearing workflow state.');
        await this.clearWorkflowState(conversationId);
        // Continue to normal chat processing below
      } else {
        // Continue the workflow for all other steps (platform selection, caption, details, approval, etc.)
